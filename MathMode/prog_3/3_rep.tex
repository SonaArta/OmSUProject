...
Задание № 3\\[1cm]
\uppercase{{\large{
	Численные методы решения краевой задачи для двумерного уравнения теплопроводности
}}}\\[3cm]
Вариант №\ 2 - 13\\[2cm]
\end{center}
...
Для всех приведенных далее результатов использовались следующие параметры моделирования:
 \begin{enumerate}
	...
	\item Временной диапазон $t \in [0.0, 5.0]$.
	\item $CFL \leq 1$ -- условие Куранта, Фридрихса, Леви.
	\item Для тестовой задачи $\lambda = 1.0$ , $c = 1.0$\\
	В индивидуальном задании  $\lambda$ и $c$ являются функциями.
\end{enumerate}

\newpage
\subsection{Решение тестовых задач}
\begin{equation}
	c(\textbf{r}) \displaystyle \frac{\partial u}{\partial t} = \displaystyle \frac{\partial}{\partial \textbf{r}} \biggl( \lambda(\textbf{r}) \displaystyle \frac{\partial u}{\partial \textbf{r}}\biggl)
\end{equation}
Область определения решения:
\begin{center}
	\begin{tabular}{c c c }
...
\begin{center}
	\begin{tabular}{c c }
		$u(x = 0, y, t) = \mu_L (y, t)$& $\mu_L (y, t) = 0$\\
		$u(x = 1, y, t) = \mu_R (y, t)$& $\mu_R (y, t) = 0$\\
		$\  \frac{\partial u }{\partial y} (x, y = 0, t) = \mu_B (x, t)$ &$\mu_B (x, t) = 0$\\
		$\  \frac{\partial u }{\partial y} (x, y = 1, t) = \mu_T (x, t)$ &$\mu_T (x, t) = 0$\\
	\end{tabular}
\end{center}
Начальные условия: $\phi (x, y) = \sin(2\pi x/(\mu_R - \mu_L))  \cos(2\pi y/(\mu_T - \mu_B))$


\begin{figure}[h!]
	\begin{center}
		\begin{minipage}[h]{0.23\linewidth}
			\includegraphics[width=\textwidth]{test_task/temp_t_0.pdf } \begin{center}	a)	\end{center}
		\end{minipage}
		\begin{minipage}[h]{0.23\linewidth}
			\includegraphics[width=\textwidth]{test_task/temp_t_5.pdf} \begin{center}	b)	\end{center}
		\end{minipage}
		\begin{minipage}[h]{0.26\linewidth}
			\includegraphics[width=\textwidth]{test_task/temp_t_10.pdf} \begin{center}	c)	\end{center}
		\end{minipage}
		\begin{minipage}[h]{0.23\linewidth}
			\includegraphics[width=\textwidth]{test_task/temp_t_15.pdf} \begin{center}	d)	\end{center}
		\end{minipage}
	\end{center}
	\caption{Решение для моментов времени a)$\ t = 0.0$, b) $\ t = 1.562$, c)$\ t = 3.125$, d)$\ t = 4.6875$.}
\end{figure}

\begin{figure}[h!]
	\begin{center}
		\begin{minipage}[h]{0.24\linewidth}
			\includegraphics[width=\textwidth]{test_task/temp_t_0_srez.pdf } \begin{center}	a)	\end{center}
		\end{minipage}
		\begin{minipage}[h]{0.24\linewidth}
			\includegraphics[width=\textwidth]{test_task/temp_t_5_srez.pdf} \begin{center}	b)	\end{center}
		\end{minipage}
		\begin{minipage}[h]{0.24\linewidth}
			\includegraphics[width=\textwidth]{test_task/temp_t_10_srez.pdf} \begin{center}	c)	\end{center}
		\end{minipage}
		\begin{minipage}[h]{0.24\linewidth}
			\includegraphics[width=\textwidth]{test_task/temp_t_15_srez.pdf} \begin{center}	d)	\end{center}
		\end{minipage}
	\end{center}
	\caption{Срезы решения для моментов времени a)$\ t = 0.0$, b) $\ t = 1.562$, c)$\ t = 3.125$, d)$\ t = 4.6875$.}
\end{figure}
...
Начальное распределение температур было задано на сетке по косинусудальному закону в направлении обеих осей таким образом, что создавались <<ячейки>> высоких и низких температур, чередующихся в шахматном порядке
с максимальными температурами $1$, $-1$. Граничные условия заданы в виде косинусоидальной зависимости температуры от пространственной 
координаты $y$, формируются чередующиеся источники высоких и низких температур. В направлении оси $x$ источники отсутствуют.
Происходит моделирование релаксации системы, установление теплового равновесия. На графиках изображена пространственнная зависимость температуры 
...
Тепловое равновесие достигается в малый промежуток времени, промежуточное состояние не отображено на графиках.
На рисунках отображен результат релаксации большей части системы: на центральной части сетки установилась равновесная температура, на левой и правой границы заметны влияния граничных условий. Видно, что влияние источников высокой температуры имеет большое значение. Это связано с тем, что длина волны косинусоиды, которая
задает граничные условия так соотносится с шириной сетки, что 
источники тепла имеют большее влияние, чем источники холода.  С течением времени усиливается влияние источников низкойтемпературы на нагретую среду.																			
