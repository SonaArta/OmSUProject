...
class MSM : public ODE_Solver<U_arg, T_arg>
{
    ...
    public:
        ...
        {
           rkm = new RKM_implicit<U_arg, T_arg>(func, u_start, t_start, dt, order);
...
U_arg MSM<U_arg,T_arg>::get_sum_multy()
{
...
template<typename U_arg, typename T_arg>
void MSM<U_arg,T_arg>::get_start_u()
{
    const int n = this->m_order - 1;
...
template<typename U_arg, typename T_arg>
U_arg MSM<U_arg,T_arg>::get_last_step()
{
    const int n = this->m_order - 1;
...
template<typename U_arg, typename T_arg>
U_arg MSM<U_arg,T_arg>::get_Solution()
{
    if(current_index < this->m_order - 1)
    {
        return all_previous_u[current_index++];
    }
    else
    {
